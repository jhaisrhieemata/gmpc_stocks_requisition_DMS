...
import { mockOfficeRequests, api } from '@/services/api';
import { BranchGroup, RequisitionItem, formatCurrency, BRANCHES, OfficeSupplyStatus, getPluralUnit } from '@/types';

const STATUS_OPTIONS: OfficeSupplyStatus[] = [
  'Pending',
  'Approved',
  'No Stocks Available',
...
export default function OfficeSuppliesPage() {
  const [requests, setRequests] = useState<BranchGroup[]>(mockOfficeRequests);
  ...
  const handleStatusChange = async (itemId: string, newStatus: OfficeSupplyStatus) => {
    // Update local state
    setRequests(prev => 
      prev.map(group => ({
        ...group,
  ...
  const handleBatchStatusChange = async (newStatus: OfficeSupplyStatus) => {
  ...
  const handlePrint = () => {
    ...
    const html = `
      ...
      <html>
        <head>
          <title>Office Supplies Report - ${new Date().toLocaleDateString()}</title>
          <style>
            ...
            .no-stock { background: #fce4ec; color: #c2185b; }
          </style>
        </head>
        <body>
          <div class="header">
            <h1>GIANT MOTO PRO</h1>
            <p class="subtitle">Office Supplies Report</p>
            ...
            <tbody>
              ${itemsToPrint
                ...
                <tr>
                  ...
                  <td><span class="status ${item.status === 'Approved' ? 'approved' : item.status === 'Cancelled' ? 'cancelled' : item.status === 'No Stocks Available' ? 'no-stock' : 'pending'}">${item.status || 'Pending'}</span></td>
                </tr>
              `
                )
                .join('')}
  ...
  const getStatusBadge = (status?: string) => {
    switch (status) {
      ...
      case 'No Stocks Available':
        return <Badge variant="destructive">No Stocks</Badge>;
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-display font-bold text-foreground">Office Supplies Requests</h1>
          <p className="text-muted-foreground">View and manage all office supplies requests</p>
        </div>
      </div>
      ...
      <Card className="shadow-lg">
        <CardHeader className="flex flex-col gap-4">
          <div className="flex flex-row items-center justify-between flex-wrap gap-4">
            <div>
              <CardTitle className="font-display">All Office Supplies Requests</CardTitle>
                ...
                <div className="flex items-center gap-2 mr-4">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-48">
                      ...
                      <DropdownMenuItem 
                        onClick={() => handleBatchStatusChange('No Stocks Available')}
                        className="gap-2"
                      >
                    ...
                    <TableCell>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                          ...
                          <DropdownMenuItem 
                            onClick={() => handleStatusChange(item.id, 'No Stocks Available')}
                            className="gap-2"
                          >
                            <PackageX className="w-4 h-4 text-destructive" />
                            No Stocks
